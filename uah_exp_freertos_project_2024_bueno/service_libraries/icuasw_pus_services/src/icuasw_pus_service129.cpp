...
// casos para 129
	switch (tcHandler.GetSubType()) {
		case (1):
		// llamada a Exec129_1TC
		Exec129_1TC(tcHandler, tmList);
			break;

		case (2):
		// llamada a Exec129_2TC
...
void PUSService129::Exec129_1TC(CDTCHandler &tcHandler, CDTMList &tmList) {
	//TODO
		// usamos GetNextFloat puesto que son tipo float
		float vx = tcHandler.GetNextFloat();
		float vy = tcHandler.GetNextFloat();

		// Igualamos valores recibidos a nuestras variables estáticas
		sCVx= vx;
		sCVy= vy;
...
void PUSService129::Exec129_2TC(CDTCHandler &tcHandler, CDTMList &tmList) {
	...
	//TODO
		...
		float Kx = tcHandler.GetNextFloat();
		float Ky = tcHandler.GetNextFloat();
		// Igualamos nuevas ctes recibidas a nuestras variables estáticas
		sKpx = Kx;
		sKpy = Ky;
...
// esta funcion limita la fuerza ejercida, por si los valores calculados son excesivos
